...
use crate::domain::units::components::{Unit, TeamId, UnitGrid, UnitMove };
...
pub fn apply_unit_spawns(
    ...
    for ev in ev_req.read().copied() {
        ...
        let e = commands
            .spawn((Unit, ev.team, UnitGrid(ev.at)))
            .id();

        // 2) 점유 시도 (경쟁 상황 대비)
        if occ.claim(ev.at, e) {
            ev_applied.write(UnitSpawnApplied { entity: e, team: ev.team, at: ev.at });
        } else {
            // 실패 시 엔티티 정리
